...
unsigned long long st_dev; 
  ...
  struct android_timespec st_atim; 
  struct android_timespec st_mtim; 
  struct android_timespec st_ctim; 
  unsigned long long st_ino; 
};

ABI_ATTR int bridge_stat(const char * pathname,struct android_stat * statbuf){
    struct stat *nbuf;
    nbuf = malloc(sizeof(struct stat));
    int ret=stat(pathname,nbuf);
   verbose("NATIVE","stat for filename %s - size %d",pathname,nbuf->st_size);
    statbuf->st_dev=nbuf->st_dev;
    statbuf->__st_ino=nbuf->st_ino;
    statbuf->st_nlink=nbuf->st_nlink;
    statbuf->st_uid=nbuf->st_uid;
    statbuf->st_gid=nbuf->st_gid;
    statbuf->st_rdev=nbuf->st_rdev;
    statbuf->st_size=nbuf->st_size;
    statbuf->st_blksize=nbuf->st_blksize;
    statbuf->st_blocks=nbuf->st_blocks;
    statbuf->st_ino=nbuf->st_ino;
    return ret;
}
...
// {
//     printf("vasprintf start %s - %p - %p\n",fmt,s,*s);
//     int ret;
//     va_list va;
//     va_start(va, fmt);
//     printf("parameters: %p - %p\n", va_arg(va, char*), va_arg(va, char*));
//     va_start(va, fmt);
//     ret = vasprintf(s, fmt, va);
//     va_end(va);
//     printf("vasprintf end ret=%d\n",ret);
//     return ret;
// }

ABI_ATTR int bridge_vasprintf(char **s, const char *fmt, va_list va)
{
    int ret;
    ret = vasprintf(s, fmt, va);
    return ret;
